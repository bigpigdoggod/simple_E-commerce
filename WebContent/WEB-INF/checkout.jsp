...
<%@ page import="cn.web.model.OrdersOrder"%>
        ...
        <div class="container-fluid custom-container">
            <div class="row">
                ...
                <div class="col-xl-4 col-lg-5">
                    <div class="checkout-total-wrapper mt-30">
                        ...
                        <div class="checkout-total mt-30">
                            ...
                            total=total.add(item.get(i).getProductsPrice().multiply(BigDecimal.valueOf(count.get(i))));%>
                                <li>
                                    <p class="total-value"><%=item.get(i).getProductsName() %> X <%=count.get(i) %></p>
                                    <p class="total-amount">€<%=item.get(i).getProductsPrice().multiply(BigDecimal.valueOf(count.get(i))) %></p>
                                </li>
                            <%} %>
                            </ul>
                    ...
                    <%List<OrdersOrder> model =new ArrayList<OrdersOrder>();
                    for(int i=0;item.size()>i;i++)
                    {
                        ...
                        String Orderid=df.format(d);
                        Orderid=userID+"-"+ Orderid;
    					OrdersOrder m=new OrdersOrder(Orderid,userID,item.get(i).getProductssId(),d,shopID,item.get(i).getProductsPrice(),item.get(i).getProductsPrice(),"Paid");
    					//订单ID，用户ID，产品ID，订单日期，商店ID，单价，总价，状态
    					model.add(m);
                    }
                    ...
                    <div class="checkout-btn">
                        <a class="main-btn main-btn-2" href="Pay">Place Order</a>
                    </div>
                </div>
            </div>
        </div>
